...
export class LobbyRoomsService {
  ...
  rejoin(lobbyUser: LobbyUser, roomId: RoomId): RoomResponse | false {
    const roomIndex = this.findRoomIndexByRoomId(roomId);
  ...
  removeUser(roomId: RoomId, socketId: SocketId): RoomResponse {
    const roomIndex = this.findRoomIndexByRoomId(roomId);
    const room = this.rooms[roomIndex];
  ...
  createMessage(user: LobbyUser, roomId: RoomId, text: string): RoomChat {
    const roomIndex = this.findRoomIndexByRoomId(roomId);
    const room = this.rooms[roomIndex];
  ...
  findRoomIndexByRoomId(roomId: RoomId): number {
